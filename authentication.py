...
def login(usernamelogin,passwordlogin):
    ...
    for user1 in user:
        if (user1["username"]==usernamelogin and user1["password"]==passwordlogin):
            if (user1["status"].lower()=='inactive'):
                return_value=10
            else: 
                if (user1["category"].lower()=="admin"):
                    return_value=1
                    break
                if (user1["category"].lower()=="owner"):
                    return_value=2
                    break
                if (user1["category"].lower()=="tenant"):
                    return_value=3
                    break
    return return_value
...
def createUser():
    ...
    while(True):
        ...
        if(category.lower() not in user_cat):
            print("Wrong category")
        else:
            break
    while(True):
        status=input("Enter the status (Active/ Inactive): ")
        if(status.lower() not in user_status):
            print("Wrong category")
        else:
            break
